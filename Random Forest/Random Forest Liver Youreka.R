...
#Liver Data
df_liver<-df_long %>%
  filter(Location == "Liver.Drug.Accumulation")
df_liver<-select(df_liver,-(5:6))

predictors = df_liver %>% select(-Accumulation_Class)

outcome = df_liver %>% pull(Accumulation_Class) %>% 
