...
#Spleen Data
df_spleen<-df_long %>%
  filter(Location == "Spleen.Drug.Accumulation")
df_spleen<-select(df_spleen,-(5:6))

predictors = df_spleen %>% select(-Accumulation_Class)

outcome = df_spleen %>% pull(Accumulation_Class) %>% 
