...
#Heart Data
df_heart<-df_long %>%
  filter(Location == "Heart.Drug.Accumulation")
df_heart<-select(df_heart,-(5:6))

predictors = df_heart %>% select(-Accumulation_Class)

outcome = df_heart %>% pull(Accumulation_Class) %>% 
