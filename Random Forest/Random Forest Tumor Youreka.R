...
#Tumor Data
df_tumor<-df_long %>%
  filter(Location == "Tumor.Drug.Accumulation")
df_tumor<-select(df_tumor,-(5:6))

predictors = df_tumor %>% select(-Accumulation_Class)

outcome = df_tumor %>% pull(Accumulation_Class) %>% 
