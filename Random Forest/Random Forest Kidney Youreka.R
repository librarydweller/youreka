...
#Kidney Data
df_kidney<-df_long %>%
  filter(Location == "Kidney.Drug.Accumulation")
df_kidney<-select(df_kidney,-(5:6))

predictors = df_kidney %>% select(-Accumulation_Class)

outcome = df_kidney %>% pull(Accumulation_Class) %>% 
